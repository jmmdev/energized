...
export default function SearchSelector({color}) {
    ...
    return (
        <div id="selection" ref={selectorRef} className={
            `flex justify-between items-center h-full relative px-2 cursor-pointer ${textColor} ${colorToPick.color} ${showOptions ? "rounded-t-xs": "rounded-xs"}`}
        ...
        style={{visibility: "hidden"}}>
            ...
            <div id="options" className={`${showOptions ? "rounded-b-sm" : "invisible"} w-max absolute z-99 top-full left-0 ${colorToPick.color} text-my-white cursor-default`}>
                {OPTIONS.map((element, index) => {
                    return (
                        <p className={`px-2 py-1 ${textColor} ${selectedIndex === index && colorToPick.hover} ${colorToPick.hover}
